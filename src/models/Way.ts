    ...
    if (graph) {
      ...
      for (const wayVertex of way) {
        ...
        if (wayVertex.plan === lastStep.plan) {
          lastStep.distance += graph.getDistanceBetween2Vertexes(lastStep.way.at(-1) as Vertex, wayVertex.id) ?? 0
          lastStep.way.push(wayVertex)
        } else {
          this.steps.push(new Step(wayVertex.plan, wayVertex))
        }
      }
      const firstStep = this.steps[0]
      if (firstStep.way.length === 1) {
        const nearestNeighborV = graph.findVertexById(firstStep.way[0].neighborData[0][0])
        if (nearestNeighborV) firstStep.way.unshift(nearestNeighborV)
      ...
      if (lastStep && lastStep.way.length === 1) {
        ...
        if (nearestNeighborV) lastStep.way.push(nearestNeighborV)
