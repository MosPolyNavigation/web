...
export class Graph {
		...
		corpusesOfLocations.forEach(corpus => {
			corpus.stairs.forEach(stairsGroup => {
				for (let stairIndex = 1; stairIndex < stairsGroup.length; stairIndex++) {
					...
					if(lowerStairVertex && upperStairVertex)
						this.addNeighborBoth(lowerStairVertex, upperStairVertex, 1085, 916)
				}
			})
		})
	}
	...
	public getShortestWayFromTo(idVertex1: VertexId, idVertex2: VertexId) {
		...
		while (finals.size !== filteredVertexes.length && !isEndVertexInFinals) {
			...
			if (currentVertex) {
				for (const [neighborVertexId, distanceToNeighbor] of currentVertex.neighborData) { //для всех айдишников соседей вершины по айди
					const neighborVertex = this.findVertexById(neighborVertexId)
					if(!neighborVertex) {
						console.error(`У вершины ${currentVertexID} в соседях есть вершина ${neighborVertexId}, но ее не существует в графе`)
						continue
					}
					if (!filteredVertexes.includes(neighborVertex))
						continue
					iterations[1] += 1
	...
	getDistanceBetween2Vertexes(vertex1: Vertex, vertex2Id: VertexId) {
		...
		if(!neighborRecord) console.error(`Попытка получить расстояние ${vertex1.id} до соседа ${vertex2Id}, но они не соседи`)
