	...
	constructor(args?: { from?: string | null | Pointer.NOTHING, to?: string | null | Pointer.NOTHING, swap?: boolean}) {
		...
		if (this.to && this.from) {
			try {
				...
				function generateUserInstructions(steps: Step[]) {
					const instructions: string[] = []
					
					steps.forEach((step, index) => {
						// Добавляем начальную точку для первого шага
						if (index === 0) {
							const firstVertex = step.way[0]
							if (firstVertex.type === 'entrancesToAu') {
								instructions.push(`${firstVertex.id} (Аудитория)`)
							}
						}
						
						// Анализируем последнюю точку текущего шага
						const lastVertex = step.way[step.way.length - 1]
						
						// Если это не последний шаг, проверяем тип последней точки
						if (index < steps.length - 1) {
							if (lastVertex.type === 'stair') {
								const nextStep = steps[index + 1]
								instructions.push(`Дойти до лестницы, подняться на ${nextStep.plan.floor}-й этаж`)
							} else if (lastVertex.type === 'crossing') {
								const nextStep = steps[index + 1]
								instructions.push(`Дойти до перехода, перейти в корпус ${nextStep.plan.corpus.title}`)
							}
						} else {
							// Для последнего шага добавляем конечную точку
							if (lastVertex.type === 'entrancesToAu') {
								instructions.push(`Дойти до аудитории`)
								instructions.push(`${lastVertex.id} (Аудитория)`)
							}
						}
					})
				...
				const routeInstructions = generateUserInstructions(steps)
				console.log('Инструкции по маршруту:', routeInstructions)
